...
//!
//! Scope: the generation primitives plus `MsdfAtlas` -- the atlas-level MSDF manager. `MsdfAtlas`
//! generates one tile per shape into an RGB32F `Texture2DArray` and records `(layer, range)` on the
//! core `Shape` via `Atlas.setShapeMsdf` (upstream's `requestMSDF`/`getMSDFTextureData` path), so the
//! MIT core carries the result without ever linking msdf-zig. Not ported: upstream's separate
//! shelf-packed RGBA8 SDF atlas (`rasterizeSDFAtlas`) and MSDF serialization.
...
const TextureData = slughorn.TextureData;
...
/// Atlas-level MSDF: generates one square MSDF tile per requested shape and packs them into an
/// RGB32F `Texture2DArray` (one shape per layer), recording each shape's layer + range on the core
/// `Shape` via `Atlas.setShapeMsdf`. Models upstream's `requestMSDF` / `getMSDFTextureData` (its
/// per-shape Texture2DArray path), but lives here so the MIT core stays MSDF-free -- the core only
/// *carries* the `(layer, range)` result; the tile store lives in this backend.
///
/// Assumes it is the sole MSDF generator for its atlas: it dedups by reading `Shape.msdf_layer`.
///
/// Divergence from upstream's tile convention: this backend's `renderMSDFTile` letterboxes the shape
/// into the square tile (upstream fills it by non-uniform scaling), so a sampler must derive tile
/// UVs from the centered sub-rect, not upstream's `(emCoord - emOrigin + range)/(emSize + 2*range)`.
pub const MsdfAtlas = struct {
    tile_size: u32,
    /// One entry per layer, in layer order: `tile_size * tile_size * 3` RGB floats, owned.
    layers: std.ArrayList([]f32) = .empty,

    pub fn init(tile_size: u32) MsdfAtlas {
        return .{ .tile_size = tile_size };
    }

    pub fn deinit(self: *MsdfAtlas, gpa: std.mem.Allocator) void {
        for (self.layers.items) |tile| gpa.free(tile);
        self.layers.deinit(gpa);
        self.* = undefined;
    }

    /// Number of tiles generated so far (= the texture array's layer count).
    pub fn layerCount(self: *const MsdfAtlas) usize {
        return self.layers.items.len;
    }

    /// Generates an MSDF tile for `key` and records `(layer, range)` on its `Shape`. If the shape
    /// already has a tile (`msdf_layer >= 0`), returns that layer without regenerating. Returns null
    /// when `key` is unknown to the atlas or the shape has no drawable geometry.
    pub fn request(self: *MsdfAtlas, gpa: std.mem.Allocator, atlas: *Atlas, key: Key, range: Slug) !?i32 {
        const shape = atlas.getShape(key) orelse return null;
        if (shape.msdf_layer >= 0) return shape.msdf_layer; // already generated by this manager
    ...
    /// Generates tiles for every key in `keys` (skipping any already generated). Returns the number
    /// of newly generated tiles.
    pub fn requestAll(self: *MsdfAtlas, gpa: std.mem.Allocator, atlas: *Atlas, keys: []const Key, range: Slug) !usize {
        const before = self.layers.items.len;
        for (keys) |key| _ = try self.request(gpa, atlas, key, range);
        return self.layers.items.len - before;
    }

    /// Packs every generated tile into an RGB32F `Texture2DArray`: `tile_size` x `tile_size`,
    /// `layerCount()` layers, row 0 = top, tiles in layer order. Caller owns the returned bytes.
    pub fn textureData(self: *const MsdfAtlas, gpa: std.mem.Allocator) TextureData {
        ...
        for (self.layers.items) |tile| {
            ...
            @memcpy(bytes[off .. off + src.len], src);
            off += src.len;
        }
        return .{ .bytes = bytes, .width = self.tile_size, .height = self.tile_size, .depth = num, .format = .rgb32f };
    }
};
